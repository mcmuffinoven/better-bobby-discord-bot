...
# Channel ID for bot to respond in
CHANNEL_ID = int(os.environ.get('CHANNEL_ID'))

log = logging.getLogger(__name__)

UTC = datetime.timezone.utc
ROUTINE_SCRAPE = datetime.time(hour=0,minute=00) # Routine Scrape Schedule
ALERT_MESSAGE = "Your product {product_name} is on sale for ${product_price}"

class Price_tracker(commands.Cog):
    '''
    Class for the commands related to price_tracker
    '''
    ...
    def scrape_product(self, product_category, product_url:str, user_name, user_id):
        '''
        Scrapes the product from the url provided from the request (bot command)
        '''
        try:
